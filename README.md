...
- Web 控制台会列出 Title、Artist、Album 任一为空的音频，可直接编辑三个字段并保存到原音频文件。
...
- 「元数据问题」：启动页面或点击「扫描空字段」后列出缺少 Title、Artist 或 Album 的歌曲；保存成功后，字段已完整的歌曲会从列表移除。

元数据编辑会优先使用 Mutagen 保存常见格式；对于 Mutagen 不支持写入的容器（例如部分 WebM），会使用 ffmpeg 的无损流复制方式保存。页面会校验文件版本，若歌曲在编辑期间被移动或修改，需要先刷新列表。
